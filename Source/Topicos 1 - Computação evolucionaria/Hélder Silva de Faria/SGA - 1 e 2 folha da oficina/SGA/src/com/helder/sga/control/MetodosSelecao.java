...
abstract class Selecao {
    ...
    public Object[] getMaiorForca() {
        ...
        for (Integer i = 0; i < ds.getResultadoAvaliacao().length; i++) {
            Object[] x = {ds.getResultadoAvaliacao()[i], i};
            al.add(x);
        }

        return (Object[]) Collections.max(al, new ComparaObject2());
    }
    ...
    public void formarCasais(Object[] MaiorForca, ArrayList<Object[]> al) {
        ...
        Collections.sort(al, new ComparaObject());

        ArrayList<Object[]> escolhidos = new ArrayList();

        escolhidos.add(MaiorForca);
        int cont = 0;
        int contPM = 0;

        for (int i = 0; i < ds.getResultadoAvaliacao().length / 2; i++) {

            escolhidos.add(al.get(cont++));
            cont = cont < al.size() ? cont : 0;

            escolhidos.add(al.get(cont++));
            cont = cont < al.size() ? cont : 0;

            Object[] p = escolhidos.get(contPM++);
            contPM = contPM < escolhidos.size() ? contPM : 0;
            ds.getPai()[i] = (Integer) p[1];
            escolhidos.remove(p);

            Object[] m = escolhidos.get(contPM++);
            contPM = contPM < escolhidos.size() ? contPM : 0;
            ds.getMae()[i] = (Integer) m[1];
            escolhidos.remove(m);
        }
    }
}
...
class SelecaoRoleta extends Selecao {
    ...
    public ArrayList<Object[]> preencheUniverso(ArrayList<Object[]> indComPorcetagem) {
        ...
        for (Object[] d : indComPorcetagem) {
            ...
            if (aux > 0) {
                int pos = str.indexOf(".") == 2 ? 1 : str.indexOf(".");
                i = Double.parseDouble(str.substring(pos + 2)) >= 0.5
                        ? Integer.parseInt(str.subSequence(0, pos).toString()) + 1 : Integer.parseInt(str.subSequence(0, pos).toString());
            } else {
                i = Double.parseDouble(str.substring(str.indexOf(".") + 2)) >= 0.5 ? 1 : 0;
            }
...
class SelecaoTorneio extends SelecaoRoleta {
    ...
    public void selecionar(DadosSelecao dss) {
        ...
        ArrayList<Object[]> al = this.calculoPorcentagem();

        if (al.size() != 0) {
            this.formaCasais(this.preencheUniverso(al));
        }
    }
}