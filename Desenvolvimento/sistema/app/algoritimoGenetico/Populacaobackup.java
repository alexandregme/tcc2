...
public class Populacaobackup {

	private Individuobackup[] individuos;
	private int tamPopulacao;

	// cria uma população com indivíduos aleatória
	public Populacaobackup(int numGenes, int tamPop) {
		tamPopulacao = tamPop;
		individuos = new Individuobackup[tamPop];
		for (int i = 0; i < individuos.length; i++) {
			individuos[i] = new Individuobackup(numGenes);
		}
	}

	// cria uma população com indivíduos sem valor, será composto posteriormente
	public Populacaobackup(int tamPop) {
		tamPopulacao = tamPop;
		individuos = new Individuobackup[tamPop];
		for (int i = 0; i < individuos.length; i++) {
			individuos[i] = null;
	...
	public void setIndividuo(Individuobackup individuo, int posicao) {
		individuos[posicao] = individuo;
	}

	// coloca um indivíduo na próxima posição disponível da população
	public void setIndividuo(Individuobackup individuo) {
		for (int i = 0; i < individuos.length; i++) {
		...
		while (trocou) {
			...
			for (int i = 0; i < individuos.length - 1; i++) {
				if (individuos[i].getAptidao() < individuos[i + 1].getAptidao()) {
					Individuobackup temp = individuos[i];
	...
	public Individuobackup getIndivduo(int pos) {
